...
| `mean_fail_rate` | average `fail_rate` across all tests present. **Caveat:** `n_tests_present` differs per config (see &sect;RQ1), so comparing this column raw *across* configs mixes different test populations and can mislead (see the worked example in RQ1 below) — prefer `mean_fail_rate_delta_vs_baseline` or the McNemar/Wilcoxon columns, which are computed on the paired population |
...
The clearest, most interpretable difference: RAFT-test failures are **~9x more likely to be classified as timeouts** than ordinary baseline-flaky failures (10.3% vs 1.2%). But this is narrower than "resource constraint causes timeouts" in general — checking `error_messages_raft.csv` by config, **77 of the 81 timeout-category occurrences (95%) come from config C (CPU throttling) specifically**, with D/M/N contributing only 1, 1, and 2 respectively. So the honest finding is "CPU-starved tests are much more likely to fail via timeout" rather than a property of resource-induced flakiness broadly — memory/disk/network-induced RAFT failures don't show this pattern. It's also worth knowing that 31 of those 81 occurrences (38%) come from a single test (`babel-preset-modules`'s integration test under C) — a concentration caveat similar to RQ2's `pypa-setuptools` finding, so this isn't 24 independent instances of evidence so much as it looks on the surface.

Assertion-mismatch-style failures, by contrast, occur at essentially the *same* rate in both groups (~15-16%) and aren't concentrated in any one config — suggesting that mechanism (order-dependency, shared mutable state, race conditions manifesting as a wrong value rather than a hang) isn't specifically a resource-constraint phenomenon; it's a general flakiness mechanism that resource pressure doesn't obviously amplify or suppress.
